 ...
 * method, multi-thread method, and a parallel stream method for determining all the prime 
 * numbers within a selected range of 1 to 2147483647.
 * @author James Santos
 * @version 4.0.1
 * 
 * Updated Information:
 ...
 *  
 *  11/14/2019
 *  - Added a new function that works exactly like determinePrimeNumbers, but instead, uses an IntStream
 *  - Created a new menu item that uses streams to determine all the prime numbers within a range
 *  - Updated the menu to incorporate the new menu item
 */

public class PrimeNumbersV4
{	
	...
	static class PossiblePrimeAwaitTermination implements Runnable {
		...
		@Override
		public void run() {
			if(PrimeNumbersV4.determinePrimeNumber(num)) {
	...
	public static ArrayList<Integer> primeInRange(int range) {
		...
		for(int i=2; i <= range; i++) {
			if(PrimeNumbersV4.determinePrimeNumber(i)) {
				primes.add(i);
			}
		}
	...
	/**
	 * Method that returns a boolean on whether or not the provided number is a prime, but
	 * with streams and lambda expressions for the predicate.
	 * @param possiblePrime - Whole Number to be determined if it is a Prime
	 * @return - True/False whether or not the number provided is a prime
	 */
	public static boolean streamIsPrime(int possiblePrime) {
		return LongStream.rangeClosed(2, possiblePrime/2).parallel().allMatch(i -> possiblePrime % i != 0);
	}
	...
	public static void menuItem1(int range, boolean displayPrimes) {
		...
		ArrayList<Integer> answersSingle = PrimeNumbersV4.primeInRange(range);
	...
	// Parallel Stream
	public static void menuItem5(int range, boolean displayPrimes) {
		
		System.out.println("Computing primes through Stream...");
		
		// Determine all prime numbers in range of argument as well as
		// determining how long it took
		long startStream = System.currentTimeMillis();
		
		// Generate the primeList
		List<Integer> primeList = IntStream.rangeClosed(2, range)
											.parallel()
											.filter(PrimeNumbersV4::streamIsPrime)
											.sorted()
											.boxed()
											.collect(Collectors.toList());
		
		// Finishes running
		long runtimeStream = System.currentTimeMillis() - startStream;
		
		// Output Results
		if(displayPrimes) {
			System.out.println(
					"Primes are " + primeList.stream()
												.map(i -> Integer.toString(i))
												.collect(Collectors.joining(", "))
			);
		}
		
		System.out.println("That took " + runtimeStream/1000F + " seconds");
		System.out.println("There are a total of " + primeList.size() + " prime numbers.");
		
	}
	
	// Change prime range
	private static int menuItem6(int num) {
	...
	// Display Prime Range
	private static void menuItem7(int num) {
		System.out.println(num +" is the number");
	}
	
	// Change pool size
	private static int menuItem8(int pool, int prime) {
	...
	// Display pool size
	private static void menuItem9(int pool) {
		System.out.println(pool + " is the pool size.");
	}
	
	// Optimize pool size
	private static int menuItem10() {
	...
	// Bypass cap size
	private static int menuItem12(int pool) {
	...
	private static boolean menuItem13(boolean primeDisplay) {
	...
	public static void main(String[] args) 
	{
		...
		while(true) {
			...
			System.out.println("Please choose an option"
					...
					+ "\n8) Change Pool Size"
					+ "\n9) Display Pool Size"
					+ "\n10) Optimize Pool Size"
					+ "\n11) Custom Optimize Pool Size"
					+ "\n12) Bypass Pool Size Limits"
					
					+ "\nOptions:"
					+ "\n13) Toggle Prime Display"
					+ "\n14) Exit");
			...
			try {
				...
				if(numDecision > 14 || numDecision < 0) {
					throw new Exception();
				}
			...
			switch(numDecision) {
				...
				case 8:
					poolSize = menuItem8(poolSize, numArg);
					break;
				...
				case 12:
					poolSize = menuItem12(poolSize);
					break;
				
				// Toggle prime display
				case 13:
					isDisplayed = menuItem13(isDisplayed);
					break;
